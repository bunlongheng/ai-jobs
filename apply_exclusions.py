...
def pat_for(t):
    if t == "java": return re.compile(r"\bjava\b", re.I)      # never matches javascript
    if t in ("angular", "angularjs"): return re.compile(r"\bangular(?:js)?\b", re.I)
    if t == "c#": return re.compile(r"c#", re.I)
    if t == "c sharp": return re.compile(r"\bc\s?sharp\b", re.I)
    if t in (".net",): return re.compile(r"\.net\b", re.I)
    if t == "dotnet": return re.compile(r"\bdotnet\b", re.I)
    # generic whole-word (handles contract, contractor, c2c, 1099, temporary, corp-to-corp...)
    return re.compile(r"(?<![a-z0-9])" + re.escape(t).replace(r"\ ", r"[\s/_-]+") + r"(?![a-z0-9])", re.I)
...
def main():
    ...
    tech = [(t, pat_for(t)) for t in (x.lower() for x in prof.get("tech_exclude", []))]
    emp = [(t, pat_for(t)) for t in (x.lower() for x in prof.get("employment_exclude", []))]
